import { ModelValue } from './getModelValue';
const mock = {
  warehouse: {
    name: "CenterOfHerbs",
    warehouseProduct: {
...
test('1 level - simple type', () => {
  ...
  const foundValue = ModelValue.get(mock, 'warehouse.name')
...
test('2 level - simple type', () => {
  ...
  const foundValue = ModelValue.get(mock, 'warehouse.warehouseProduct.count')
...
test('3 level - simple type', () => {
  ...
  const foundValue = ModelValue.get(mock, 'warehouse.warehouseProduct.product.name')
...
test('4 level - Array', () => {
  ...
  const foundValue = ModelValue.get(mock, 'warehouse.warehouseProduct.product.barcodes')
...
test('4 level - Object', () => {
  ...
  const foundValue = ModelValue.get(mock, 'warehouse.warehouseProduct.product')
